...
#' Max Time of flight, Minimum length, and maximum length.
#'
#' @param ch Channel data
#' @param MaxTOF Maximum time of flight
#' @param MinLength Minimum length
#' @param MaxLength Maximum length
#'
#' @return Returns the index number of the filtered objects
#' @export
FilterChannel <- function(ch,MaxTOF, MinLength,MaxLength) {
  ...
  if (!missing(MaxTOF) & !missing(MinLength)) {
    WormNumber <- which(ch > MaxTOF, arr.ind = TRUE)
    ...
    WormNumber <- unique(which(ch > MaxTOF, arr.ind = TRUE))
